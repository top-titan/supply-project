...
export const getFamilyDashboardData = async (req: Request, res: Response) => {
  try {
    ...
    const daysUntilDelivery = family.nextDeliveryDate
      ? Math.ceil(
          (new Date(family.nextDeliveryDate).getTime() - new Date().getTime()) /
            (1000 * 3600 * 24)
        )
      : null;
...
export const getFamilyConsumptionData = async (req: Request, res: Response) => {
  try {
...
export const getFamilyActivities = async (req: Request, res: Response) => {
  try {
